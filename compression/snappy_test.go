...
func TestSnappyCompressor_Compress(t *testing.T) {
	snappyCompressor := NewSnappy()

	e := &linked_list.Element{
		Value:       []byte(payload),
		EnqueueTime: time.Time{},
	}
	err := snappyCompressor.Compress(e)

	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}

	if len(payload) >= len(e.Value) {
		t.Fatalf("Expected compressed data to be smaller than original data")
	}
}

func TestSnappyCompressor_Decompress(t *testing.T) {
	snappyCompressor := NewSnappy()
	data := &linked_list.Element{
		Value:       []byte(payload),
		EnqueueTime: time.Time{},
	}
	err := snappyCompressor.Compress(data)
	//element := &linked_list.Element{Value: pressed}

	err = snappyCompressor.Decompress(data)

	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}

	if !reflect.DeepEqual(data.Value, payload) {
		t.Fatalf("Expected decompressed data to be equal to original data")
	}
}

//
//func BenchmarkSnappyCompressor_Compress(b *testing.B) {
//	snappyCompressor := NewSnappy()
//	data := []byte(payload) // Large data for better benchmark
//
//	b.ResetTimer() // Exclude the setup time
//
//	for i := 0; i < b.N; i++ {
//		_, _ = snappyCompressor.Compress(data)
//	}
//}
//
//func BenchmarkSnappyCompressor_Decompress(b *testing.B) {
//	snappyCompressor := NewSnappy()
//	data := []byte(payload) // Large data for better benchmark
//	compressed, _ := snappyCompressor.Compress(data)
//	element := &linked_list.Element{Value: compressed}
//
//	b.ResetTimer() // Exclude the setup time
//
//	for i := 0; i < b.N; i++ {
//		_ = snappyCompressor.Decompress(element)
//	}
//}
