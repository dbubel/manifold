...
//
//	for i := 0; i < b.N; i++ {
//		_, err := compressor.CompressBufPool(data)
//		if err != nil {
//			b.Errorf("CompressIOPool error: %v", err)
//		}
//	}
//}
...
func BenchmarkGZIP(b *testing.B) {
	...
	// First, we compress data
	//if err != nil {
	//	b.Fatalf("CompressIOPool error: %v", err)
	//}

	b.ResetTimer() // Resets the timer to exclude the setup time

	for i := 0; i < b.N; i++ {
		compressedData, err := compressor.Compress(GenerateRandomString(lenth))
		_, err = compressor.Decompress(compressedData)
		if err != nil {
			b.Errorf("DecompressIOPool error: %v", err)
		}
	}
}
