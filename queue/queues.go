...
func (q Queues) Enqueue(id string, value []uint8) error {
	queue, ok := q[id]
	if !ok {
...
func (q Queues) Dequeue(id string) ([]byte, error) {
	queue, ok := q[id]
	if !ok {
		// Create a new queue if it doesn't exist yet
		queue = NewQueue()
...
func (q Queues) BlockingDequeue(ctx context.Context, id string) ([]byte, error) {
	queue, ok := q[id]
	if !ok {
		// Create a new queue if it doesn't exist yet
		queue = NewQueue()
